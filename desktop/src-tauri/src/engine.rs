...
impl Engine {
    ...
    /// Edit the TEXT of one of my posts/comments, leaving its attachments alone.
    ///
    /// An Edit event carries a FULL media array and the reducer REPLACES with it (see
    /// `EventKind::Edit` in haven-p2p `social.rs`) — it is not a patch. This used to pass `vec![]`,
    /// which meant editing a typo in a caption silently detached every photo on the post for the
    /// whole circle. So the current media/music/mute state is read back and re-sent unchanged; only
    /// the body moves. Apple has always done this (`EditPostSheet` sends the full array).
    pub fn edit_post(self: &Arc<Self>, circle_id: String, target: String, body: String) {
        let current = self
            .reoptimize_targets()
            .into_iter()
            .find(|t| t.circle_id == circle_id && t.event_id == target);
        let (media, music, mute_video) = match current {
            Some(t) => (t.media, t.music, t.mute_video),
            // Not found = the item carries no media at all (reoptimize_targets only lists those that
            // do), so an empty array is the correct and unchanged value.
            None => (vec![], None, false),
        };
        if let Ok(env) =
