...
final class FeedStore: ObservableObject {
    ...
    /// The engine, behind its actor (see `Engine`). This is the ONLY reference to the social engine
    /// in the app, and the only way to use it is `await engine.run { … }` — so nothing on the main
    /// actor can call it synchronously, and every result comes back as a value that is assigned
    /// here. Every `Task` that runs a pass captures the instance it started with and checks
    /// `self.engine === engine` before applying, so a pass that outlives a `reconfigure` is dropped.
    private var engine: Engine? {
        didSet { dropEngineCaches() }   // the read model follows the engine instance
    }
    // MARK: - Read model
    //
    // Everything a view (or another store) reads SYNCHRONOUSLY comes from here: plain values the
    // engine passes produce off the main actor and hand back. The engine itself is never consulted
    // on the main thread. A cache miss answers with what it has (the last snapshot, or nothing) and
    // schedules the pass that fills it; the pass publishes when it lands, so a view that asked
    // re-renders with the answer. Before this, each of these was a lazy "read the engine on first
    // use" fallback, and every one of them was a place the main thread could park behind the
    // engine's unfair mutex (stall detector, 2026-09-01: 2.05s in `dialTargets → myNodeHex`, 2.59s
    // in `WatchSessionManager.buildSnapshot → dmPartnerName → contactNodeIds`, 4.09s in
    // `handleRelayNode → contactNodeIds`).
    //
    // Identity: fixed the moment `configure` adopts it — read once by the boot pass.
    private var cachedAccountHex = ""
    private var cachedDeviceHex = ""
    /// My public bundle — constant for the life of an identity.
    private var cachedBundle = Data()
    /// My signed profile card, minted by the boot pass, re-minted by every sync pass and by
    /// `rebroadcastProfile`. `helloPayload` builds hellos from it without touching the engine.
    private var cachedProfileCard = Data()
    /// circle id → member ACCOUNT ids. Filled by every circles read (the boot pass, `reloadCircles`,
    /// the sync pass, the DM purge); a miss schedules one.
    private var circleMembersCache: [String: [String]] = [:]
    /// lowercased account id → dialable DEVICE ids from their signed roster, or `[account]` while we
    /// hold none (the engine's own fallback shape). Filled by the boot pass and the sync pass, and
    /// re-read after every roster ingest; a miss schedules a refill.
    private var deviceIdsCache: [String: [String]] = [:]
    private var membersFillInFlight = Set<String>()
    private var deviceIdsFillInFlight = false
    private func dropEngineCaches() {
        cachedAccountHex = ""; cachedDeviceHex = ""; cachedBundle = Data(); cachedProfileCard = Data()
    ...
    private func scheduleMembersFill(_ circleId: String) {
        ...
        Task { @MainActor [weak self] in
            ...
            self.adoptMembers([circleId: m])
        }
    }
    /// Merge freshly-read members; a change invalidates the dial cache and publishes so the DM
    /// helpers (thread titles, call buttons) repaint with the answer.
    private func adoptMembers(_ members: [String: [String]], publish: Bool = true) {
        var changed = false
        for (cid, m) in members where circleMembersCache[cid] != m {
            circleMembersCache[cid] = m
            dialTargetsCache.removeValue(forKey: cid)
            changed = true
        }
        if changed {
    ...
    /// Dial ids for an account from the read model. A miss answers the engine's own fallback (the
    /// account id) and schedules a refill.
    private func cachedDeviceIds(_ accountHex: String) -> [String] {
        let key = accountHex.lowercased()
        if let ids = deviceIdsCache[key] { return ids }
        scheduleDeviceIdsFill()
        return [accountHex]
    }
    /// Re-read every known account's device ids in ONE pass — contacts, members, me — cheap map
    /// lookups in the engine. Coalesced; runs after roster ingests and on a cache miss.
    private func scheduleDeviceIdsFill() {
        guard let engine, !deviceIdsFillInFlight else { return }
        deviceIdsFillInFlight = true
        var accounts = Set(ContactsStore.shared.contacts.map { $0.idHex.lowercased() })
        accounts.insert(myNodeHex.lowercased())
        ...
        Task { @MainActor [weak self] in
            ...
            guard let self else { return }
            self.deviceIdsFillInFlight = false
            guard self.engine === engine else { return }
            self.adoptDeviceIds(ids)
        }
    }
    /// Merge freshly-read device ids; a change invalidates the dial cache so the new ids dial now.
    private func adoptDeviceIds(_ ids: [String: [String]]) {
        var changed = false
        for (a, d) in ids where deviceIdsCache[a.lowercased()] != d { deviceIdsCache[a.lowercased()] = d; changed = true }
        if changed { dialTargetsCache.removeAll(); ownDeviceHexCache = (0, []) }
    }
    /// Engine mutations that must land BEFORE a later pass may use their result — `startDM` creates
    /// the circle a `sendMessage` a moment later posts into. Actor jobs from two unstructured Tasks
    /// carry no ordering guarantee, so the later pass awaits the earlier one by circle id.
    private var pendingCircleOps: [String: Task<Void, Never>] = [:]
    private func awaitPendingCircleOp(_ circleId: String) async {
        if let t = pendingCircleOps[circleId] { await t.value }
    }
    ...
    ///
    /// The boot runs entirely off the main actor, in two engine passes: `bootEngine` (construct +
    /// identity + import + register) and `postBootPass` (everything `finishConfigure` used to do to
    /// the engine on main — the default circle, the superseded-circle heal, the removal reconcile,
    /// the crypto switches — plus the first read-model snapshot: identity, bundle, profile card,
    /// circles, members, device ids). `finishConfigure` then publishes the snapshot and continues on
    /// the main actor in the same order as before. Until then `engine` is nil — every entry point
    /// guards on it (`engineReady` says so) — and a caller that needs the engine next passes
    /// `then:`, which runs after the boot (immediately when it already exists).
    private var configureInFlight = false
    private var configureCompletions: [() -> Void] = []
    func configure(mode: BootMode, then: (() -> Void)? = nil) {
        guard engine == nil, !configureInFlight else {
            if engine != nil { then?() } else if let then { configureCompletions.append(then) }
            return
        }
        ...
        let inputs = postBootInputs(mode: mode)
        Task(priority: .userInitiated) { @MainActor [weak self] in
            // Construction (identity key derivation) is real work too — off the main actor, like
            // every engine call after it.
            let core: HavenSocial? = await Task.detached(priority: .userInitiated) {
                Self.constructEngine(mode: mode)
            }.value
            var engine: Engine?
            ...
            if let core {
                let e = Engine(core)
                await e.run { s in
                    Self.bootEngine(s, mode: mode, deviceSeed: deviceSeed, deviceBundle: deviceBundle, deviceName: deviceName,
                                    rosterWire: rosterWire, stateURL: stateURL, isDemo: isDemo)
                }
                snapshot = await e.run { s in Self.postBootPass(s, inputs) }
                engine = e
            }
            guard let self else { return }
            self.configureInFlight = false
            await self.finishConfigure(engine, snapshot: snapshot, mode: mode)
        }
    }

    /// Construct the engine handle for `mode` (no identity adoption, no import — see `bootEngine`).
    nonisolated private static func constructEngine(mode: BootMode) -> HavenSocial? {
        switch mode {
        case .seeded(let seed): return try? HavenSocial(accountSeed: seed)
        case .seedless(let bundle, let dSeed): return try? HavenSocial.newSeedless(accountPublicBundle: bundle, deviceSeed: dSeed)
        }
    }

    /// Identity + import + register, on the engine actor. The ORDER is unchanged from the
    /// synchronous version and matters — see the comments inline.
    nonisolated private static func bootEngine(_ social: HavenSocial, mode: BootMode, deviceSeed: Data, deviceBundle: Data,
                                               deviceName: String, rosterWire: Data?, stateURL: URL, isDemo: Bool) {
        let seedless: Bool
        switch mode {
        case .seeded:
            seedless = false
        ...
        case .seedless:
            ...
            if let rosterWire, !rosterWire.isEmpty { _ = social.ingestRosterWire(wire: rosterWire) }
        }
    ...
    /// Main-actor state the post-boot engine pass needs, snapshotted before it runs.
    private struct PostBootInputs {
        var seedless = false
        /// Holds the account master seed (may create the default circle, pin creators, retire the leaf).
        var seedHolder = false
        var keepMyPosts = true
        var activeCircleId = "default"
        var deletedCircleIds = Set<String>()
        /// "<circleId>|<hex>" client-side severances (`ConnectionsStore.circleRemovals`).
        var circleRemovals = Set<String>()
        /// Circles this account created (`CircleCreatorStore`) — the ones whose creator pin is re-applied.
        var createdCircleIds = Set<String>()
        var contactIds: [String] = []
        var profile: (name: String, bio: String, link: String, avatar: String, emoji: String) = ("", "", "", "", "")
        /// §1 migration gate: not yet flagged retired, roster enabled — see `postBootPass`.
        var tryRetireAccountLeaf = false
    }
    /// What the post-boot pass hands back: the read model's first snapshot plus what it changed.
    private struct BootSnapshot {
        var accountHex = ""
        var deviceHex = ""
        var bundle = Data()
        var profileCard = Data()
        var circles: [CircleInfoFfi] = []
        var members: [String: [String]] = [:]
        var deviceIds: [String: [String]] = [:]
        var activeSuccessor: String?
        var createdDefaultCircle = false
        /// Superseded circles left behind (legacy id → successor) — tombstoned on main.
        var healedSuperseded: [(legacy: String, successor: String?)] = []
        var healedRemovals = 0
        /// §1 migration outcome: the roster wire to rebroadcast when the leaf was retired just now,
        /// and whether it was found already retired.
        var retiredLeafWire: Data?
        var leafAlreadyRetired = false
    }
    private func postBootInputs(mode: BootMode) -> PostBootInputs {
        var i = PostBootInputs()
        if case .seedless = mode { i.seedless = true }
        i.seedHolder = AccountStore.storedSeed() != nil
        i.keepMyPosts = SettingsStore.shared.keepMyPosts
        i.activeCircleId = activeCircleId
        i.deletedCircleIds = CircleDeletionStore.deletedIds()
        i.circleRemovals = ConnectionsStore.shared.circleRemovals
        i.createdCircleIds = CircleCreatorStore.createdIds()
        i.contactIds = ContactsStore.shared.contacts.map(\.idHex)
        let ps = ProfileStore.shared
        i.profile = (name: ps.displayName.isEmpty ? "Someone" : ps.displayName, bio: ps.bio, link: ps.link,
                     avatar: ps.avatarBase64, emoji: ps.emoji)
        i.tryRetireAccountLeaf = !i.seedless && !SwitchFlipMigration.accountLeafRetired
            && i.seedHolder && DeviceRosterManager.shared.isEnabled
        return i
    }

    /// Everything `finishConfigure` used to do to the engine on the main actor, in the same order,
    /// plus the first read-model snapshot. One engine pass, off-main.
    nonisolated private static func postBootPass(_ s: HavenSocial, _ i: PostBootInputs) -> BootSnapshot {
        var out = BootSnapshot()
        s.setKeepOwnPosts(on: i.keepMyPosts)   // apply the archive preference
        // A brand-new identity must have its own "default" circle to post into. The "default"
        // circle ("Your circle") was only ever created as a side effect of accepting your first
        // connection, which left a brand-new user's very first post silently dropped — `post` on a
        // circle the engine doesn't hold returns nil and the composer clears with nothing to show.
        // Seed-holders only; a seedless device receives it from the primary's roster sync.
        let me = s.myNodeHex()
        if i.seedHolder, !s.circles().contains(where: { $0.id == "default" }) {
            s.createCircle(id: "default", name: "Your circle")
            _ = s.setCircleCreator(circleId: "default", accountHex: me)
            out.createdDefaultCircle = true
        }
        // The superseded-circle heal (see `reloadCircles`) — the tombstones are written on main.
        for legacy in s.supersededCircleIds() where !i.deletedCircleIds.contains(legacy) {
            let succ = s.circleSuccessor(circleId: legacy)
            s.leaveCircle(id: legacy)
            out.healedSuperseded.append((legacy, succ))
        }
        // Self-heal old-build damage. A build BEFORE the authoritative-tombstone fix could re-add a
        // member you'd removed — the automatic add cleared the ENGINE tombstone while the CLIENT
        // tombstone stayed set. The result: the member is hidden in the UI (client filter) yet still
        // lives in the engine's member list, so posts/sealing/roster still treat them as present and
        // it "never sticks". Purge any engine member that's still client-tombstoned — re-running the
        // removal that was undone. Idempotent; a no-op once the fleet is on the fixed build.
        for key in i.circleRemovals {
            ...
            if s.contactNodeIds(circleId: circleId).contains(hex) {
                s.removeFromCircle(circleId: circleId, nodeHex: hex)   // purge + re-tombstone in the engine
                out.healedRemovals += 1
            }
        }
        // Switch-Flip 1.0.7 (docs/SWITCH-FLIP-1.0.7.md): re-apply the NON-PERSISTED crypto switches
        // every launch (§3/§4/§2/§5). Every switch is gated in core — a circle stays byte-identical
        // to 1.0.6 until its whole membership is capable — so this is safe to run unconditionally. A
        // seedless device sets keying + DM live-lanes but never the primary-only ops (seed-drop
        // retirement, account-leaf migration): the primary that holds the account key owns those.
        // §3 MLS keying master switch — every device (off→shadow→live handled by the all-joined gate).
        s.setMlsKeying(on: true)
        // §4 seed-drop retirement — PRIMARY only (a seedless device holds no bare account key to drop).
        if !i.seedless { s.setSeedDropRetire(on: true) }
        ...
        for c in s.circles() {
            ...
            if c.id.hasPrefix("dm:") { s.setCircleLiveLane(circleId: c.id, on: true) }
            ...
            if !i.seedless, c.id == "default" || i.createdCircleIds.contains(c.id) {
                _ = s.setCircleCreator(circleId: c.id, accountHex: me)
            }
        }
        // §1 MIGRATION — retire the bare account leaf so the roster reaches the DEVICE-ONLY shape that
        // live MLS keying and account-key retirement require. Gated + idempotent in core (a no-op until
        // the fleet is fully device-capable), and guarded by a one-time flag on main so a migrated
        // roster isn't rebroadcast every launch. Runs only on a seed-holding primary with multi-device
        // enabled; a brand-new device-only install technically never needs it, but the call is
        // harmless there (it just marks the sticky flag).
        if i.tryRetireAccountLeaf {
            if s.retireAccountLeaf() {
                out.retiredLeafWire = s.myDeviceRosterWire()
            } else if s.accountLeafRetired() {
                out.leafAlreadyRetired = true   // already retired on a prior launch / another device
            }
        }
        // The read model's first snapshot.
        out.accountHex = me
        out.deviceHex = s.myDeviceNodeHex()
        out.bundle = s.myBundle()
        out.profileCard = s.mySignedProfile(name: i.profile.name, bio: i.profile.bio, link: i.profile.link,
                                            avatar: i.profile.avatar, emoji: i.profile.emoji)
        out.circles = s.circles()
        var accounts = Set(i.contactIds.map { $0.lowercased() })
        accounts.insert(me.lowercased())
        for c in out.circles {
            let m = s.contactNodeIds(circleId: c.id)
            out.members[c.id] = m
            for a in m { accounts.insert(a.lowercased()) }
        }
        for a in accounts where !a.isEmpty { out.deviceIds[a] = s.deviceNodeIdsFor(accountHex: a) }
        out.activeSuccessor = s.circleSuccessor(circleId: i.activeCircleId)
        return out
    }

    /// The main-actor half of `configure`: publish the engine and its first snapshot, then
    /// everything that used to follow the boot, in the same order.
    private func finishConfigure(_ booted: Engine?, snapshot: BootSnapshot?, mode: BootMode) async {
        defer {
        ...
        if let snapshot {
            ...
            if snapshot.createdDefaultCircle { CircleCreatorStore.markCreated("default") }
        }
        ...
        if let snapshot {
            // Publish circles + members + device ids from the boot pass (what `refreshCircles` /
            ...
            if let wire = snapshot.retiredLeafWire {
                SwitchFlipMigration.accountLeafRetired = true
                // Rebroadcast the now device-only roster wire to my own devices (and, via the normal
                // circle / self-sync roster export, to friends) — higher-version-wins supersedes the legacy roster.
            ...
            } else if snapshot.leafAlreadyRetired {
        ...
        if appIsForeground, let engine { MediaBackupQueue.shared.drainPersisted(engine: engine) }
        await seedDemoIfNeeded()   // HAVEN_DEMO=1 only — PII-free synthetic dataset for screenshots
    ...
    private func dailyMailboxRefreshIfDue() {
        ...
        NotificationCenter.default.addObserver(forName: SharedStore.rosterEpochChangedNotification,
            ...
            Task { @MainActor in
                ...
                self.backfillMailbox(circleIds: self.circles.map(\.id))
            }
        }
    }
    ...
    /// The engine, exposed so `DemoSeeder` can drive the real seal→open→feed pipeline with synthetic
    /// friend identities (on the engine actor, like everything else). Nil until `configure` has run.
    var demoEngine: Engine? { engine }
    ...
    private func seedDemoIfNeeded() async {
        ...
        await DemoSeeder.seed(feed: self)
    ...
    /// Re-read the circle list (and every circle's members) and publish. Fire-and-forget for the
    /// synchronous callers; `reloadCircles()` is the awaitable form for a caller that must see the
    /// new list before continuing (self-sync's post-merge pull, the circle mutations).
    func refreshCircles() {
        Task { @MainActor [weak self] in await self?.reloadCircles() }
    }

    /// One circles read, as the engine pass hands it back.
    private struct CirclesRead {
        var circles: [CircleInfoFfi]
        var members: [String: [String]]
        /// Superseded circles the pass left behind (legacy id → successor) — tombstoned on main.
        var healed: [(legacy: String, successor: String?)]
        /// The successor of the active circle, if it has one (consulted when it is tombstoned).
        var activeSuccessor: String?
    }
    private var circlesReloadGeneration: UInt64 = 0
    func reloadCircles() async {
        guard let engine else { circles = []; return }
        scheduleDMIntruderPurge()       // clean DM membership every time we read circles (off-main)
        circlesReloadGeneration &+= 1
        let gen = circlesReloadGeneration
        let deleted = CircleDeletionStore.deletedIds()
        let active = activeCircleId
        let read: CirclesRead = await engine.run { s in
            ...
            var healed: [(legacy: String, successor: String?)] = []
            for legacy in s.supersededCircleIds() where !deleted.contains(legacy) {
                let succ = s.circleSuccessor(circleId: legacy)   // capture BEFORE leaving (lookup needs the circle)
                s.leaveCircle(id: legacy)                        // drop the duplicate row from the engine
                healed.append((legacy, succ))
            }
            let all = s.circles()
            var members: [String: [String]] = [:]
            for c in all { members[c.id] = s.contactNodeIds(circleId: c.id) }
            return CirclesRead(circles: all, members: members, healed: healed,
                               activeSuccessor: s.circleSuccessor(circleId: active))
        }
        guard self.engine === engine else { return }
        // A newer reload superseded this one: its tombstones still apply (the engine already left
        // those circles), its list does not.
        if circlesReloadGeneration != gen {
            for h in read.healed { CircleDeletionStore.markDeleted(h.legacy) }
            return
        }
        applyCirclesRead(read)
    }
    /// Publish one circles read: tombstone what the pass healed, publish the deduped list, refill
    /// the members cache, and move off a deleted active circle.
    private func applyCirclesRead(_ read: CirclesRead) {
        var changed = false
        for h in read.healed {
            CircleDeletionStore.markDeleted(h.legacy)                 // LWW tombstone → syncs to all my devices
            if activeCircleId == h.legacy, let succ = h.successor { activeCircleId = succ }
            changed = true
        }
        ...
        let list = read.circles.filter { !CircleDeletionStore.isDeleted($0.id) && seenCircleIds.insert($0.id).inserted }
        let listChanged = circles != list
        if listChanged { circles = list }
        adoptMembers(read.members, publish: !listChanged)   // the `circles` assignment already published
        ...
        if CircleDeletionStore.isDeleted(activeCircleId) {
            activeCircleId = read.activeSuccessor ?? "default"
        }
    ...
    /// in one engine pass, and the pass refreshes the members cache for every circle it visits.
    ...
    private func scheduleDMIntruderPurge() {
        guard let engine, !dmPurgeInFlight else { return }
        dmPurgeInFlight = true
        Task { @MainActor [weak self] in
            let (fixed, members): (Bool, [String: [String]]) = await engine.run { s in
                var fixed = false
                var members: [String: [String]] = [:]
                for circle in s.circles() {
                    var m = s.contactNodeIds(circleId: circle.id)
                    if circle.id.hasPrefix("dm:") {
                        var purged = false
                        for nodeHex in m where !Self.dmCircleAllows(circle.id, nodeHex) {
                            s.removeFromCircle(circleId: circle.id, nodeHex: nodeHex)
                            purged = true
                        }
                        if purged { fixed = true; m = s.contactNodeIds(circleId: circle.id) }
                    }
                    members[circle.id] = m
                }
    ...
    // (A fresh identity's "default" circle is created by the boot pass — see `postBootPass`.)
    ...
    func pendingUpgrades(_ circleId: String) async -> [CircleUpgradeOffer] {
        guard let engine else { return [] }
        return await engine.run { $0.pendingCircleUpgrades(circleId: circleId) }
    }
    ...
    func circleIsUpgradable(_ circleId: String) async -> Bool {
        guard !circleId.hasPrefix(OwnedCircle.prefix), circleId != "default", !circleId.hasPrefix("dm:"),
              let engine else { return false }
        return await engine.run { $0.circleAdmins(circleId: circleId).isEmpty }
    }
    ...
    @discardableResult
    func offerCircleUpgrade(_ circleId: String) async -> String? {
    ...
    func createCircle(name: String, memberIds: [String] = []) {
        Task { @MainActor in await self.createCircleNow(name: name, memberIds: memberIds) }
    }
    /// The awaitable form (the QA driver dumps after it): the new circle's id once it exists.
    @discardableResult
    func createCircleNow(name: String, memberIds: [String] = []) async -> String? {
        ...
        let id: String = await engine.run { s in
            let id = s.createCircleOwned(name: name)
            for m in memberIds { try? s.addExistingToCircle(circleId: id, nodeHex: m) }
            return id
        }
    ...
    func clearCircleRemovalEverywhere(idHex: String, circleId: String) {
        ...
        if let engine {
            Task { @MainActor in await engine.run { $0.clearCircleRemoval(circleId: circleId, nodeHex: idHex) } }
        }
    ...
    func addContactToActiveCircle(idHex: String) {
        ...
        ConnectionsStore.shared.clearCircleRemoval(idHex, circleId: cid)  // deliberate re-add un-bans them
        forceHelloNextSync(idHex, circleId: cid)   // the invite rides the hello — never warm-skip it
        Task { @MainActor [weak self] in
            await engine.run { s in
                s.clearCircleRemoval(circleId: cid, nodeHex: idHex)          // …and lift the engine tombstone
    ...
    @discardableResult
    func removeFromAllCircles(_ idHex: String) async -> Int {
        guard let engine else { return 0 }
        let removed: [String] = await engine.run { s in
            var out: [String] = []
            for c in s.circles() where !c.id.hasPrefix("dm:") && s.contactNodeIds(circleId: c.id).contains(idHex) {
                s.removeFromCircle(circleId: c.id, nodeHex: idHex)          // purge + engine tombstone + epoch rotate
                out.append(c.id)
            }
            return out
        }
    ...
    // (The launch-time removal reconcile — purging engine members that are still client-tombstoned,
    // old-build damage — runs in the boot pass; see `postBootPass`.)
    ...
    func nonContactMembers(in circleId: String) -> [String] {
        ...
        let myHex = myNodeHex
        return cachedMembers(circleId).filter { $0 != myHex && !mine.contains($0) }
    }
    ...
    func dialTargets(_ circleId: String) -> [String] {
        ...
        // Cache miss: build from the read model (members + device ids). The sync pass pre-seeds every
        // circle from its off-main `SyncReads` snapshot, so this path is the exception, not the 20s
        // tick — and a read-model miss inside it schedules the fill and invalidates this cache when
        // it lands, so a brand-new member becomes dialable within one pass.
        let members = cachedMembers(circleId)
        var deviceIds: [String: [String]] = [:]
        for a in members { deviceIds[a.lowercased()] = cachedDeviceIds(a) }
    ...
    func publishAccountDevices() {
        guard let node, let engine else { return }
        Task.detached {
            do {
                let ids = try await engine.publishAccountDevices(via: node)
    ...
    func leaveActiveCircle() {
        guard activeCircleId != "default", let engine else { return }
        let cid = activeCircleId
        CircleDeletionStore.markDeleted(cid)   // LWW tombstone so a sibling can't re-create it
        Task { @MainActor [weak self] in
    ...
    func startDM(with idHex: String, name: String) -> String {
        ...
        guard let engine else { return id }
        CircleDeletionStore.markRecreated(id)   // re-opening a DM lifts any prior deletion (LWW)
        forceHelloNextSync(idHex, circleId: id)
        // The id is deterministic, so the caller can navigate to it now; the circle itself lands a
        // pass later. Anything that posts into it next (`sendMessage`) awaits this op first.
        pendingCircleOps[id] = Task { @MainActor [weak self] in
            await engine.run { s in
                s.createCircle(id: id, name: name)
                s.setCircleLiveLane(circleId: id, on: true)   // Switch-Flip §5: DM per-message forward secrecy
                try? s.addExistingToCircle(circleId: id, nodeHex: idHex)
            }
            guard let self else { return }
    ...
    func startGroupDM(members: [String], name: String) -> String {
        ...
        let others = members.filter { $0 != myNodeHex }
        pendingCircleOps[id] = Task { @MainActor [weak self] in
            await engine.run { s in
                s.createCircle(id: id, name: name)
                s.setCircleLiveLane(circleId: id, on: true)   // Switch-Flip §5: DM per-message forward secrecy
                for hex in others { try? s.addExistingToCircle(circleId: id, nodeHex: hex) }
            }
            guard let self else { return }
            self.pendingCircleOps[id] = nil
            guard self.engine === engine else { return }
    ...
    func approveConnection(_ req: ConnectionRequest) {
        Task { @MainActor in await self.approveConnectionNow(req) }
    }
    /// The awaitable form (the QA driver's `approve_connections` dumps after it).
    func approveConnectionNow(_ req: ConnectionRequest) async {
        guard let engine else { return }
        ...
        // Lift the engine tombstone, verify + add their bundle, and re-seal my history for them
        // (`syncEnvelopes` is a hybrid signature per event — it ran on main before this) in one pass.
        let (vhex, history): (String?, [Data]) = await engine.run { s in
            s.clearCircleRemoval(circleId: "default", nodeHex: req.idHex)   // lift the engine tombstone too
            let vhex = try? s.bundleVerificationHex(bundle: req.bundle)
            s.createCircle(id: "default", name: "Your circle")
    ...
    /// Per-circle feed SNAPSHOTS — the read model behind every chat body (`ordered`), the thread
    /// list's previews, the Watch snapshot and the badge/notify paths. `feed()` is a full decrypt
    /// under the engine mutex; before the read model it ran on main on a 2s TTL, and the field
    /// beachball was the Messages view body + `notifyNewest` each calling it while utility workers
    ...
    /// Mark a circle's snapshot stale (or every snapshot). The old items stay readable until the
    /// re-read lands, so a thread never flashes empty on its own send.
    private func invalidateMessagesCache(_ circleId: String? = nil) {
        if let circleId {
            messagesStale.insert(circleId)
            messagesGeneration[circleId, default: 0] &+= 1
        } else {
            for k in messagesCache.keys { messagesStale.insert(k); messagesGeneration[k, default: 0] &+= 1 }
        }
    }
    ...
    private func storeMessages(_ circleId: String, _ items: [FeedItemFfi], readAt: UInt64, generation: UInt64? = nil,
        ...
        if messagesCache.count > 40 {
            ...
            if messagesCache.count > 40 {
                ...
                for e in oldest where e.key != circleId { messagesCache.removeValue(forKey: e.key) }
            }
        }
        if changed && publish {
            objectWillChange.send()
            // A DM thread's snapshot just moved: the Messages badge counts from these snapshots, so
            // recount now rather than on the next ingest (the old TTL re-read did this incidentally).
            if circleId.hasPrefix("dm:") { recomputeUnreadDMs() }
        }
    }
    private func scheduleMessagesLoad(_ circleId: String) {
        guard let engine, messagesLoadInFlight.insert(circleId).inserted else { return }
        ...
        Task { @MainActor [weak self] in
            ...
            guard self.engine === engine else { return }
            self.storeMessages(circleId, items, readAt: nowMs, generation: gen)
        }
    }
    ...
    private func warmDMThreads() async {
        ...
        let nowMs = now()
        let feeds: [String: [FeedItemFfi]] = await engine.run { s in
    ...
    func post(_ postId: String, in circleId: String) async -> FeedItemFfi? {
        guard let engine else { return nil }
        let nowMs = now()
        return await engine.run { s in
            let items = s.feed(circleId: circleId, nowMs: nowMs, viewerRetentionSecs: nil)
            return items.first { $0.id == postId }
                ?? items.first { $0.comments.contains { $0.id == postId } }
        }
    }

    /// Messages of a circle (for a DM thread) without disturbing the main feed — from the read
    /// model. A circle with no snapshot yet (or a stale one) answers what it has and schedules the
    /// read; the pass publishes when it lands. Safe to call from a view body: nothing here
    ...
    func messages(in circleId: String) -> [FeedItemFfi] {
        ...
        let hit = messagesCache[circleId]
        if hit == nil || messagesStale.contains(circleId) { scheduleMessagesLoad(circleId) }
        let all = hit?.items ?? []
    ...
    func sendMessage(to circleId: String, _ body: String, media rawMedia: [String], music: TrackRefFfi?, retentionSecs: UInt64? = nil) -> Bool {
        ...
        Task { @MainActor in _ = await self.authorMessage(to: circleId, body, media: rawMedia, music: music, retentionSecs: retentionSecs) }
        return true
    }
    /// The awaitable form: true once the message is sealed and handed to every transport (the
    /// share-sheet drain and the QA driver wait for this; the UI fires and forgets).
    @discardableResult
    func authorMessage(to circleId: String, _ body: String, media rawMedia: [String], music: TrackRefFfi?, retentionSecs: UInt64? = nil) async -> Bool {
        let media = withPreviewMarkers(withThumbMarkers(rawMedia))
        let ts = now()
        guard let engine else { return false }
        await awaitPendingCircleOp(circleId)   // a DM opened a moment ago must exist before we post into it
        guard let a = await authorEvent(engine, circleId: circleId, createdAt: ts, { s in
            try s.post(circleId: circleId, body: body, media: media, music: music, retentionSecs: retentionSecs, story: false, muteVideo: false, createdAt: ts)
        }), self.engine === engine else { return false }
    ...
    func editMessage(in circleId: String, _ id: String, _ body: String,
        ...
        Task { @MainActor [weak self] in
            guard let self, let a = await self.authorEvent(engine, circleId: circleId, createdAt: nil, { s in
                try s.edit(circleId: circleId, target: id, body: body, media: media, music: music, muteVideo: false, createdAt: ts)
            }), self.engine === engine else { return }
            let name = self.circles.first(where: { $0.id == circleId })?.name ?? "your circle"
            self.broadcastEvent(circleId, a, banner: .forEdit(circleId: circleId, circleName: name, postId: id)); self.postTick += 1; self.refresh()
        }
    }
    ...
    func deleteMessage(in circleId: String, _ id: String) {
        ...
        Task { @MainActor [weak self] in
            guard let self, let a = await self.authorEvent(engine, circleId: circleId, createdAt: nil, { s in
                try s.unsend(circleId: circleId, target: id, createdAt: ts)
            }), self.engine === engine else { return }
            self.broadcastEvent(circleId, a, banner: .forUnsend(circleId: circleId, postId: id)); self.postTick += 1; self.refresh()
        }
    }

    /// Delete a whole DM conversation locally (also clears any old contaminated thread).
    func deleteConversation(_ circleId: String) {
        guard let engine, circleId.hasPrefix("dm:") else { return }
    ...
    func maybePurgeExpiredMedia(_ circleId: String, retention: UInt64?) {
        guard let engine, !DemoEnv.isDemo, !purgedCircles.contains(circleId) else { return }
        ...
        Task { @MainActor [weak self] in
            let (purged, inUseFinal): ([String], Set<String>) = await engine.run { s in
                let purged = s.purgeExpired(circleId: circleId, viewerRetentionSecs: retention, nowMs: nowMs)
                ...
                var inUse = Self.mediaInUseStems(s)
                for r in scheduledRefs { inUse.formUnion(MediaStore.storedStems(for: r)) }
                inUse.formUnion(pinnedStems)
                return (purged, inUse)
            }
            guard !purged.isEmpty, let self, self.engine === engine else { return }
            self.invalidateMessagesCache(circleId)   // the purged events are gone from the snapshot too
    ...
    nonisolated static func mediaInUseStems(_ social: HavenSocial) -> Set<String> {
    ...
    func cleanupUnusedMedia(grace: TimeInterval = 3600) async -> (bytes: Int64, files: Int) {
        guard let engine, !DemoEnv.isDemo else { return (0, 0) }
        let scheduledRefs = ScheduledStore.shared.items.flatMap(\.media)
        let pinnedStems = PinnedMediaStore.shared.inUseStems()   // device-pinned blobs are cleanup-exempt
        let inUse: Set<String> = await engine.run { s in
            var inUse = Self.mediaInUseStems(s)
            for r in scheduledRefs { inUse.formUnion(MediaStore.storedStems(for: r)) }
            inUse.formUnion(pinnedStems)
            return inUse
        ...
        let result = await Task.detached(priority: .utility) { () -> (Int64, Int) in
            MediaStore.performOrphanSweep(inUse: inUse, graceSeconds: grace)
        }.value
        if result.1 > 0 {
    ...
    func mediaInventory() async -> [MediaInventoryRow] {
        ...
        let owner: [String: (circleId: String, snippet: String, ts: UInt64)] = await engine.run { s in
            var owner: [String: (circleId: String, snippet: String, ts: UInt64)] = [:]
            let nowMs = UInt64(Date().timeIntervalSince1970 * 1000)
            for c in s.circles() {
                for item in s.feed(circleId: c.id, nowMs: nowMs, viewerRetentionSecs: nil) {
    ...
    func deleteSelectedMedia(_ rows: [MediaInventoryRow]) async -> Int64 {
        guard let engine else { return 0 }
        let inUse = await engine.run { Self.mediaInUseStems($0) }
        var freed: Int64 = 0
    ...
    func reoptimizeTargets() async -> [ReoptimizeTarget] {
        guard let engine else { return [] }
        let nowMs = now()
        return await engine.run { s in
    ...
    /// waiting for them.
    @discardableResult
    func applyReoptimized(_ target: ReoptimizeTarget, media: [String]) async -> Bool {
        guard let engine else { return false }
        let ts = now()
        guard let a = await authorEvent(engine, circleId: target.circleId, createdAt: nil, { s in
            try s.edit(circleId: target.circleId, target: target.eventId,
                       body: target.body, media: media, music: target.music,
                       muteVideo: target.muteVideo, createdAt: ts)
        }), self.engine === engine else { return false }
    ...
    func enforceLocalLimits(force: Bool = false) {
        guard !DemoEnv.isDemo, let engine, !limitSweepInFlight else { return }
        ...
        Task { @MainActor [weak self] in
            ...
            let r = await Task.detached(priority: .utility) {
                MediaStore.performLimitSweep(maxDays: maxDays, maxGB: maxGB, pinnedStems: pinnedStems, inUse: inUse)
            }.value
            guard let self else { return }
    ...
    var myNodeIdShort: String { engine == nil ? "—" : String(myNodeHex.prefix(16)) }
    /// ACCOUNT id, from the read model (set by the boot pass; "" until the engine is up).
    var myNodeHex: String { cachedAccountHex }
    ...
    var myDeviceNodeHex: String { cachedDeviceHex }
    ...
    /// the engine actor inside ActivityStore; deduped by event id, so calling liberally is cheap.
    func pullActivity() {
        guard let engine else { return }
        ActivityStore.shared.pull(engine: engine)
    }
    ...
    private func processMatrixQaDropFile() {
        ...
        if obj["op"] is String {
            Task { @MainActor in await self.applyMatrixQaV2(obj) }
            return
        }
        var items: [String: String] = [:]
        for (k, v) in obj {
    ...
    private func applyMatrixQa(_ items: [String: String]) {
        guard engine != nil else {
            HavenLog.net("matrix-qa deferred — engine not ready")
            return
        }
        ...
        // Every op awaits its engine pass before the dump, so the dump reflects the op.
        let publish: ([String]) async -> Void = { [weak self] refs in
            guard let self else { return }
            if let body = items["post"], !body.isEmpty {
                await self.postNow(body, media: refs)
            ...
            if let body = items["story"], !body.isEmpty {
                if refs.isEmpty {
                    await self.postNow(body, media: [], music: nil, retentionSecs: 86_400, story: true)
                } else {
                    await self.postStoryNow(media: refs, caption: body)
                }
            ...
            if dmTo.count == 64, !dmBody.isEmpty {
                await self.qaSendDM(to: dmTo, body: dmBody, refs: refs)
            }
    ...
    private func applyMatrixQaV2(_ obj: [String: Any]) async {
        guard engine != nil else {
            HavenLog.net("matrix-qa v2 deferred — engine not ready")
            return
        }
        ...
        if ["post", "story", "file", "music_post"].contains(op) {
            ...
            if !target.isEmpty {
                if circles.contains(where: { $0.id == target }) {
        ...
        case "approve_connections":
            ...
            for r in reqs { await approveConnectionNow(r) }
        ...
        case "post", "story", "dm":
            ...
            var refs: [String] = []
            if wantsMedia {
                refs = await Self.qaBuildMediaRefs(photoPath: photoPath, videoPath: videoPath, kind: mediaKind)
                if refs.isEmpty {
                    HavenLog.net("matrix-qa v2 media FAILED — no refs from kind=\(mediaKind)")
                }
            }
            // The op's engine pass completes before the dump below, so the dump reflects it.
            switch op {
            case "post":
                await postNow(body, media: refs)
            case "story":
                let caption = str("caption").isEmpty ? body : str("caption")
                if refs.isEmpty {
                    await postNow(caption, media: [], music: nil, retentionSecs: 86_400, story: true)
                } else {
                    await postStoryNow(media: refs, caption: caption)
                }
            case "dm":
                await qaSendDM(to: str("dm_to").lowercased(), body: body, refs: refs)
            default: break
            }
            qaMarkUserActive(mutating: true)
            qaWriteDump()
            return
        ...
        case "comment":
            ...
            if !target.isEmpty, !body.isEmpty, let cid = await qaFindCircleId(ofEvent: target) {
                await commentMessageNow(in: cid, target, body)
            } else {
        ...
        case "circle_create":
            let name = str("name")
            if !name.isEmpty { await createCircleNow(name: name) }   // same engine call the UI uses

        case "circle_invite":
            let cid = str("circle_id"), hex = str("dm_to").lowercased()
            if !cid.isEmpty, hex.count == 64, let engine {
                ...
                await engine.run { s in
                    s.clearCircleRemoval(circleId: cid, nodeHex: hex)
        ...
        case "file":
            ...
            if !path.isEmpty, FileManager.default.fileExists(atPath: path) {
                ...
                else { await postNow(body, media: [ref]) }
            } else {
        ...
        case "music_post":
            ...
            await postNow(body, media: [], music: TrackRefFfi(catalogId: "qa-\(title)", title: title, artist: artist,
        ...
        case "mark_read":
            ...
            if !cid.isEmpty {
                ...
                for c in circles where c.id.hasPrefix("dm:") { DMReadStore.shared.markRead(c.id) }
            }
    ...
    private func qaSendDM(to dmTo: String, body: String, refs: [String]) async {
        ...
        _ = await authorMessage(to: cid, body, media: refs, music: nil)
        persist(); refresh()
    ...
    private func qaFindCircleId(ofEvent target: String) async -> String? {
        guard let engine, !target.isEmpty else { return nil }
        let nowMs = now()
        return await engine.run { s in
            for c in s.circles() {
                for item in s.feed(circleId: c.id, nowMs: nowMs, viewerRetentionSecs: nil)
    ...
    private func qaWriteDump() {
        ...
        let nowMs = now()
        Task { @MainActor [weak self] in
            // The diagnostics JSON (delivery + tree chain) rides the same pass — those two used to be
            // read on the main actor after the hop back, which was an engine lock on main per dump.
            let (snapshot, delivery, treeChain): ([QaCircleSnapshot], String, String) = await engine.run { s in
                let snap = s.circles().map { c in
                    QaCircleSnapshot(circle: c,
                                     items: s.feed(circleId: c.id, nowMs: nowMs, viewerRetentionSecs: nil),
                                     members: s.contactNodeIds(circleId: c.id))
                }
                return (snap, s.diagDeliveryJson(), s.debugTreeChainJson())
            }
            guard let self, self.qaDumpGeneration == gen else { return }
            self.qaWriteDumpFile(snapshot, accountHex: self.myNodeHex, tsMs: nowMs,
                                 delivery: delivery, treeChain: treeChain)
        }
    }
    ...
    private func ingestQaPeerBundle() {
        ...
        Task { @MainActor [weak self] in
            let result: Result<String, Error> = await engine.run { s in
                Result { try s.addContactBundle(circleId: "default", bundle: bundle) }
            }
            guard let self, self.engine === engine else { return }
            switch result {
    ...
    func refresh() {
        ...
        let nowMs = now()
        let messagesGen = messagesGeneration[circleId] ?? 0
        Task { @MainActor [weak self] in
            ...
            // (real crypto, takes the engine lock), and refresh() fires on every sync tick /
            // ingest. It's event-driven instead: the ingest side effects (DM), markThreadRead
            // (read), and configure (startup).
        }
    }
    ...
    func sensitiveRefs(circleId: String) -> Set<String> {
        ...
        scheduleModerationFill(circleId)
        return []
    }
    private var moderationFillInFlight = Set<String>()
    /// The sensitive set + the reports of a circle, from the engine, into the read model (the active
    /// circle's ride every `refresh` pass; any other circle fills on its first ask).
    private func scheduleModerationFill(_ circleId: String) {
        ...
        Task { @MainActor [weak self] in
            ...
            guard let self else { return }
            self.moderationFillInFlight.remove(circleId)
            guard self.engine === engine else { return }
    ...
    func flagSensitive(circleId: String, ref: String) {
        ...
        Task { @MainActor [weak self] in
            guard let self, let a = await self.authorEvent(engine, circleId: circleId, createdAt: nil, { s in
                try s.flagSensitive(circleId: circleId, target: ref, createdAt: ts)
            }), self.engine === engine else { return }
            self.broadcastEvent(circleId, a, silent: true)   // not user-facing news
    ...
    func setKeepOwnPosts(_ on: Bool) {
        guard let engine else { return }
        Task { @MainActor [weak self] in
            await engine.run { $0.setKeepOwnPosts(on: on) }
            guard let self, self.engine === engine else { return }
            self.invalidateMessagesCache()
            self.refresh()
        }
    ...
    @discardableResult
    func report(circleId: String, target: String, reason: String, comment: String) async -> String? {
        guard let engine else { return nil }
        let ts = now()
        guard let a = await authorEvent(engine, circleId: circleId, createdAt: nil, { s in
            try s.report(circleId: circleId, target: target, reason: reason, comment: comment, createdAt: ts)
        }), self.engine === engine else { return nil }
        broadcastEvent(circleId, a, silent: true)   // deliver the report without a lock-screen banner
        HiddenStore.shared.hide(target)
        // The report just authored names the author (resolved by the core from the event log).
        let grouped: [String: [ReportFfi]] = await engine.run { s in
            Dictionary(grouping: s.reports(circleId: circleId), by: { $0.target })
        }
        guard self.engine === engine else { return nil }
        reportsCache[circleId] = grouped
        let author = grouped[target]?.first?.author
    ...
    private func enqueueAuthoredMedia(_ media: [String], circleId: String, engine: Engine) {
        ...
        for ref in MediaVariants.allPreviews(in: media) + MediaVariants.allThumbs(in: media)
            ...
            MediaBackupQueue.shared.enqueue(ref, circleId: circleId, engine: engine, priority: true)
        }
    }

    /// What every authored event needs from the engine, read in ONE pass: the sealed envelope, the
    /// id the engine derived for it (the banner's deep link — only asked for when `createdAt` is
    /// given), and the broadcast inputs: the member list and the epoch HEAD that rides along the
    /// mailbox upload. The seal is a hybrid PQ signature — real crypto — and it used to run on the
    /// main actor for every post, comment and reaction.
    struct Authored {
        let env: Data
        let eventId: String?
        let members: [String]
    ...
    private func authorEvent(_ engine: Engine, circleId: String, createdAt: UInt64?,
        ...
        await engine.run { s -> Authored? in
            ...
            return Authored(env: env, eventId: id, members: s.contactNodeIds(circleId: circleId),
                            epochHeads: s.exportEpochHead(circleId: circleId))
        }
    }
    ...
    func post(_ body: String, media rawMedia: [String] = [], music: TrackRefFfi? = nil, retentionSecs: UInt64? = nil, story: Bool = false, muteVideo: Bool = false) {
        Task { @MainActor in
            await self.postNow(body, media: rawMedia, music: music, retentionSecs: retentionSecs, story: story, muteVideo: muteVideo)
        }
    }
    /// The awaitable form of `post`: the new post's id once it is sealed and handed to every transport
    /// (nil when the engine is down or the seal failed). The QA driver dumps after it.
    @discardableResult
    func postNow(_ body: String, media rawMedia: [String] = [], music: TrackRefFfi? = nil, retentionSecs: UInt64? = nil, story: Bool = false, muteVideo: Bool = false) async -> String? {
        let media = withPreviewMarkers(withThumbMarkers(rawMedia))
        let ts = now()
        let cid = activeCircleId
        guard let engine else { return nil }
        await awaitPendingCircleOp(cid)
        guard let a = await authorEvent(engine, circleId: cid, createdAt: ts, { s in
            try s.post(circleId: cid, body: body, media: media, music: music, retentionSecs: retentionSecs, story: story, muteVideo: muteVideo, createdAt: ts)
        }), self.engine === engine else { return nil }
    ...
    /// rescan's worklist. The feed walk runs on the engine actor; the local-file check on main.
    func ownUntaggedVideoPosts() async -> [(postId: String, circleId: String, ref: String)] {
        guard let engine else { return [] }
        let cids = circles.map(\.id)
        let nowMs = now()
        let found: [(postId: String, circleId: String, ref: String)] = await engine.run { s in
    ...
    func attachSongCredit(postId: String, circleId: String, track: TrackRefFfi) async -> Bool {
        ...
        let nowMs = now()
        let item: FeedItemFfi? = await engine.run { s in
            s.feed(circleId: circleId, nowMs: nowMs, viewerRetentionSecs: nil).first { $0.id == postId }
        }
        guard let item else { return false }
        guard item.music == nil else { return true }   // a song is already there — nothing to attach, nothing to retry
        let ts = now()
        guard let a = await authorEvent(engine, circleId: circleId, createdAt: nil, { s in
            try s.edit(circleId: circleId, target: postId, body: item.body,
                       media: item.media, music: track,
                       muteVideo: item.muteVideo, createdAt: ts)
        }), self.engine === engine else { return false }
        broadcastEvent(circleId, a, silent: true)
        ...
        invalidateMessagesCache(circleId)   // a DM thread re-reads on its next paint
        return true
    }
    ...
    func postImported(circleId: String, body: String, media rawMedia: [String],
                      music: TrackRefFfi? = nil, story: Bool = false, createdAt: UInt64) async {
        let media = withPreviewMarkers(withThumbMarkers(rawMedia))
        guard let engine else { return }
        guard let a = await authorEvent(engine, circleId: circleId, createdAt: createdAt, { s in
            try s.post(circleId: circleId, body: body, media: media, music: music, retentionSecs: nil,
                       story: story, muteVideo: false, createdAt: createdAt)
        }), self.engine === engine else { return }
        // The engine derives ids at author time; the pass read it back so a deferred song credit can
        // find this exact post later (see ShazamRetryQueue).
        lastImportedPostId = a.eventId
        broadcastEvent(circleId, a, silent: true)
    ...
    func postScheduled(circleId: String, body: String, media rawMedia: [String]) {
        ...
        guard let engine else { return }
        Task { @MainActor [weak self] in
            guard let self, let a = await self.authorEvent(engine, circleId: circleId, createdAt: ts, { s in
    ...
    func post(_ body: String, toCircle circleId: String) {
        ...
        Task { @MainActor [weak self] in
            guard let self, let a = await self.authorEvent(engine, circleId: circleId, createdAt: ts, { s in
                try s.post(circleId: circleId, body: body, media: [], music: nil, retentionSecs: nil, story: false, muteVideo: false, createdAt: ts)
            }), self.engine === engine else { return }
            let name = self.circles.first(where: { $0.id == circleId })?.name ?? "your circle"
            self.broadcastEvent(circleId, a, banner: .forPost(circleId: circleId, circleName: name, body: body, media: [], story: false, postId: a.eventId))
            self.postTick += 1; self.refresh()
        }
    }
    ...
    /// The awaitable form of `postStory` (the QA driver dumps after it).
    @discardableResult
    func postStoryNow(media: [String], caption: String = "", music: TrackRefFfi? = nil) async -> String? {
        await postNow(caption, media: CameraView.withPosterCompanions(media), music: music,
                      retentionSecs: 86_400, story: true)
    }
    ...
    func comment(_ id: String, _ body: String, _ rawMedia: [String] = []) {
        let cid = activeCircleId
        Task { @MainActor in
            await self.commentMessageNow(in: cid, id, body, rawMedia)
            self.noteInterestIfIncomplete(id)
        }
    }
    func react(_ id: String, _ emoji: String) {
        let cid = activeCircleId
        Task { @MainActor in
            await self.reactMessageNow(in: cid, id, emoji)
            self.noteInterestIfIncomplete(id)
        }
    }
    ...
    func unreact(_ id: String, _ emoji: String) { unreactMessage(in: activeCircleId, id, emoji) }
    /// React to a message in a specific (DM) circle.
    func reactMessage(in circleId: String, _ id: String, _ emoji: String) {
        Task { @MainActor in await self.reactMessageNow(in: circleId, id, emoji) }
    }
    /// The awaitable form of `reactMessage` (the QA driver dumps after it).
    func reactMessageNow(in circleId: String, _ id: String, _ emoji: String) async {
        guard let engine else { return }
        let ts = now()
        guard let a = await authorEvent(engine, circleId: circleId, createdAt: nil, { s in
            try s.react(circleId: circleId, target: id, emoji: emoji, createdAt: ts)
        }), self.engine === engine else { return }
        broadcastEvent(circleId, a, banner: .forReaction(emoji: emoji, circleId: circleId, postId: id))
    ...
    func unreactMessage(in circleId: String, _ id: String, _ emoji: String) {
        ...
        Task { @MainActor [weak self] in
            guard let self, let a = await self.authorEvent(engine, circleId: circleId, createdAt: nil, { s in
                try s.unreact(circleId: circleId, target: id, emoji: emoji, createdAt: ts)
            }), self.engine === engine else { return }
            self.broadcastEvent(circleId, a, silent: true); self.reactionTick += 1; self.refresh()
        }
    }
    ...
    func commentMessage(in circleId: String, _ id: String, _ body: String, _ rawMedia: [String] = []) {
        Task { @MainActor in await self.commentMessageNow(in: circleId, id, body, rawMedia) }
    }
    /// The awaitable form of `commentMessage` (the QA driver dumps after it).
    func commentMessageNow(in circleId: String, _ id: String, _ body: String, _ rawMedia: [String] = []) async {
    ...
    func edit(_ id: String, _ body: String, media rawMedia: [String] = [], music: TrackRefFfi? = nil, muteVideo: Bool = false) {
        ...
        let ts = now()
        Task { @MainActor [weak self] in
            guard let self, let a = await self.authorEvent(engine, circleId: cid, createdAt: nil, { s in
    ...
    func unsend(_ id: String) {
        ...
        Task { @MainActor [weak self] in
            guard let self, let a = await self.authorEvent(engine, circleId: cid, createdAt: nil, { s in
                try s.unsend(circleId: cid, target: id, createdAt: ts)
            }), self.engine === engine else { return }
            self.broadcastEvent(cid, a, banner: .forUnsend(circleId: cid, postId: id)); self.refresh()
        }
    }
    ...
    /// one engine pass off-main instead. Stall detector, 2026-09-01: `bringOnline → syncWithContacts
    ...
    private func syncWithContactsNow() {
        ...
        Task { @MainActor [weak self] in
            let reads = await engine.run { s in
                Self.gatherSyncReads(s, circleIds: circleIds, contactIds: contactIds, mineAcct: mineAcct, profile: profile)
            }
            // A newer pass superseded this snapshot, or the engine was replaced meanwhile: drop it.
            guard let self, self.syncPassGeneration == gen, self.engine === engine else { return }
            // Refill the read model from the pass: members, device ids, and the profile card the
            // hellos below are built from (the same bytes `helloPayload(reads:)` uses).
            self.adoptMembers(reads.members)
            self.adoptDeviceIds(reads.deviceIds)
            if !reads.bundle.isEmpty { self.cachedBundle = reads.bundle }
            if !reads.profileCard.isEmpty { self.cachedProfileCard = reads.profileCard }
            for cid in circleIds {
    ...
    private func syncWithContactsApply(_ reads: SyncReads) {
        ...
        if resendHistory {
            ...
            Task { @MainActor [weak self] in
                let freshSealed: [(cid: String, gen: UInt64, envs: [Data])] = await engine.run { s in
            ...
            if !due.isEmpty {
                ...
                Task { @MainActor in
                    ...
                    for hex in due {
                        ...
                        await SharedStore.fetchContactRoster(accountHex: hex, engine: engine)
                    }
                    self.scheduleDeviceIdsFill()   // any roster that landed changes who is dialable
                }
            }
        }
        ...
        if !thermalBlocksCatchup,
           ...
           !myOtherDeviceTargets(ownDeviceIds: reads.deviceIds[myNodeHex.lowercased()] ?? []).isEmpty {
            ...
            Task { @MainActor [weak self] in
                let workFinal: [(String, [Data])] = await engine.run { s in
                    var work: [(String, [Data])] = []
                    for cid in cidsForDevices {
                        let envs = s.exportRecentEnvelopes(circleId: cid, limit: UInt32(catchupLimit))
    ...
    func reannounceOwnRelay() {
        guard let engine else { return }
        // Snapshot membership + announce plaintext on main (cheap), seal OFF main on the engine
        // actor — sealCircleMedia is real crypto and used to run for every circle×relay
        ...
        Task { @MainActor [weak self] in
            ...
            let sealed: [SealedFrame] = await engine.run { s in
                var out: [SealedFrame] = []
                for j in jobs {
                    guard let sealed = try? s.sealCircleMedia(circleId: j.circleId, data: j.plain) else { continue }
                    var p = Data()
    ...
    private func nearbyPeerConnected() {
        ...
        #endif
        let circleSnap = circles.map { (id: $0.id, name: $0.name) }
        Task { @MainActor [weak self] in
            // FIRST: offer this device's sealed self-sync slot to nearby peers. ONLY our own devices
            // (same seed) can open it — it's how a linked Mac/phone bootstraps circles + profile +
            // posts LOCALLY, with no relay or S3 at all (the local "handshake" sync). Sent before the
            // post events below so the receiver learns the circles before their posts arrive.
            let slot = await SelfSyncCoordinator.shared.sealedLocalSlot(engine: engine)
            // The re-seal (`exportRecentEnvelopes` is real crypto per envelope) ran on the main actor
            // for every circle on every connect; it rides the engine actor now.
            let recent: [(cid: String, envs: [Data])] = await engine.run { s in
            ...
            for circle in circleSnap {
                ...
                if circle.id == "default" { self.nearbyBroadcast(0, hello, class: .control) }
                // DMs + RECEIVED events — sealed; non-members drop. Rate limiter paces bulk sends.
                for env in recent.first(where: { $0.cid == circle.id })?.envs ?? [] {
                    self.nearbyBroadcast(1, self.eventPayload(circle.id, env), class: .bulk)
                }
            }
            self.pushOwnMediaNearby(freshPeer: true)   // paced bulk media via rate limiter
            self.refresh()
        }
    ...
    private func handleNearbySelfSync(_ payload: Data) {
        Task { @MainActor in
            if await SelfSyncCoordinator.shared.ingestPeerSlot(payload, engine: engine) {
                await reloadCircles()   // a newly-synced circle must enter the polled list + pull its history
            }
        }
    }
    ...
    /// My full public bundle bytes (what a ticket's issue step parses) — from the read model.
    func myPublicBundle() -> Data? { cachedBundle.isEmpty ? nil : cachedBundle }
    ...
    @discardableResult
    func ingestInviteHello(_ body: Data) async -> Bool {
        let r = await handleHello(body, viaNearby: false, senderDevice: nil)
    ...
    /// A hello from the read model: my bundle (constant) + my signed profile card (minted by the
    /// boot pass, re-minted by every sync pass and by `rebroadcastProfile`). No engine touch.
    private func helloPayload(circleId: String, circleName: String) -> Data? {
        guard engine != nil, !cachedBundle.isEmpty, !cachedProfileCard.isEmpty else { return nil }
        var p = Data()
        lpAppend(&p, Data(circleId.utf8))
        lpAppend(&p, Data(circleName.utf8))
        lpAppend(&p, cachedBundle)
        // rest = signed business card (name + bio + link)
        p.append(cachedProfileCard)
        return p
    }
    ...
    func rebroadcastProfile() {
        guard let engine else { return }
        // Mint the fresh card first (a signature — engine work), then fan the hellos out from it.
        let ps = ProfileStore.shared
        let name = ps.displayName.isEmpty ? "Someone" : ps.displayName
        let bio = ps.bio, link = ps.link, avatar = ps.avatarBase64, emoji = ps.emoji
        let circleSnap = circles.map { (id: $0.id, name: $0.name) }
        Task { @MainActor [weak self] in
            let card = await engine.run { $0.mySignedProfile(name: name, bio: bio, link: link, avatar: avatar, emoji: emoji) }
            guard let self, self.engine === engine else { return }
            if !card.isEmpty { self.cachedProfileCard = card }
            for circle in circleSnap {
                guard let hello = self.helloPayload(circleId: circle.id, circleName: circle.name) else { continue }
                for hex in self.dialTargets(circle.id) { self.sendIroh(0, hello, to: hex) }
            }
        }
    }
    ...
    private func broadcastEvent(_ circleId: String, _ authored: Authored, silent: Bool = false,
        ...
        if notifJSON.isEmpty || silent {
            for nodeHex in members {
                PushManager.shared.wake(nodeHex, ciphertext: nil, event: eventB64, silent: silent)
            }
        } else if let engine {
            // Seal + SIGN the banner to each recipient; the relay forwards it blind, their NSE
            // decrypts AND verifies it really came from us (audit H2). One signature per member —
            // engine work, so it rides the actor; the wakes go out as the pass lands. Nothing below
            // waits on it: the event's own delivery and the mailbox upload are already on their way.
            Task { @MainActor in
                let sealed: [(hex: String, ciphertext: String?)] = await engine.run { s in
                    members.map { ($0, (try? s.sealSignedNotification(recipientNodeHex: $0, data: notifJSON))?.base64EncodedString()) }
                }
                for r in sealed { PushManager.shared.wake(r.hex, ciphertext: r.ciphertext, event: eventB64, silent: false) }
            }
        }
    ...
    func nudgeSelfSyncSoon() {
        ...
        selfSyncNudgeTask = Task { @MainActor [weak self] in
            ...
            if let slot = await SelfSyncCoordinator.shared.sealedLocalSlot(engine: self.engine) {
    ...
    func prefetchPush(circleId: String, mailboxKey: String?, mediaRefs: [String]) async {
        guard engine != nil, circles.contains(where: { $0.id == circleId }) else { return }
        ...
        Task { @MainActor in
            ...
            for item in items where !MediaStore.shared.has(item.ref) {
                ...
                let opened: Data? = await engine.run { s in
                    for cid in cids {
                        if let d = s.openCircleMedia(circleId: cid, sealed: item.sealed) { return d }
                    }
                    return nil
                }
    ...
    private func selfSyncAndPull() {
        ...
        Task { @MainActor in
            if await SelfSyncCoordinator.shared.sync(engine: self.engine, force: force) {
                // reloadCircles() — NOT just refresh() — so a circle synced from another of MY devices
    ...
    /// early on 0).
    ///
    /// SINGLE-FLIGHT. Keys are marked seen when the ingest BATCH runs (on the engine
    /// actor), but the fetch-time filter reads the seen-set immediately — so two overlapping
    ...
    /// polls outran the batches and the engine actor's queue grew without bound: 55 GB and a jetsam
    ...
    @MainActor func pullMailbox(circleIds ids: [String]) async -> Int {
        ...
        guard let engine else { return 0 }
        let moved = await engine.run { $0.takeEpochMovedCircles() }
        guard self.engine === engine else { return 0 }
        if !moved.isEmpty {
            let now = Date()
        ...
        guard ids.contains(where: { SharedStore.hasMailbox($0) }) else { return 0 }
        ...
        for (cid, key, data) in sorted where key.contains("/__hello__/") {
            ...
            let outcome = await handleHello(data, viaNearby: false, senderDevice: nil)
    ...
    func ingestPushInbox() {
        ...
        Task { @MainActor [weak self] in
            let ingestedFinal: [(circleId: String, envelope: Data)] = await engine.run { s in
                var ingested: [(circleId: String, envelope: Data)] = []
                for env in envs {
                    for cid in ids where (try? s.receive(circleId: cid, envelope: env)) == true {
    ...
    private func sendIroh(_ type: UInt8, _ payload: Data, to nodeHex: String) {
        ...
        //
        // The expansion comes from the read model (the sync pass keeps it fresh); a miss reads the
        // engine on its actor and feeds the cache. This fires dozens of times per sync burst, and it
        // used to take the engine mutex on main per send (stall detector, 2026-09-01: 0.98s parked in
        // `sendIroh → deviceNodeIdsFor → Mutex::lock`). (Per-send logging stays removed: building
        // those strings on every send was itself measurable churn during sync.)
        let engine = self.engine
        let cached = deviceIdsCache[nodeHex.lowercased()]
        let hints = deviceHints(for: nodeHex)   // typed mirror — O(1), no lock
        Task.detached(priority: .userInitiated) { [weak self] in
            var targets: [String]
            if let cached { targets = cached }
            else if let engine {
                targets = await engine.run { $0.deviceNodeIdsFor(accountHex: nodeHex) }
                let learned = targets
    ...
    private func myOtherDeviceTargets() -> [String] {
        guard engine != nil else { return [] }
        return myOtherDeviceTargets(ownDeviceIds: cachedDeviceIds(myNodeHex))
    }
    ...
    private func liveDeliverManyToMyDevices(_ type: UInt8, _ payloads: [Data]) {
        guard !payloads.isEmpty, let node, engine != nil else { return }
        let frames = payloads.map { frame(type, $0) }
        let mineAcct = myNodeHex.lowercased()
        let mineDev = myDeviceNodeHex.lowercased()
        let hints = deviceHints(for: mineAcct)
        // The own roster comes from the read model (stall detector, 2026-09-01: 0.49s parked in
        // `askForMedia → liveDeliverToMyDevices → myOtherDeviceTargets → Mutex::lock` when it was
        // an engine read here).
        let targets = Self.otherDeviceTargets(ownDeviceIds: cachedDeviceIds(mineAcct), hints: hints, mineAcct: mineAcct, mineDev: mineDev)
    ...
    private func handleInbound(_ data: Data, viaNearby: Bool, senderDevice: String? = nil) {
        ...
        if callFrameTypes.contains(type) {
            ...
            // engine mutex for as long as any storm held it (beachball sample, b349). The crypto
            // runs on the engine actor, then back to the main actor to dispatch. Consecutive frames
            ...
            guard let engine else {
                HavenLog.call("call frame type=\(type) DROPPED — no engine")
                return
            }
            Task { [weak self] in
                guard let self else { return }
                ...
                let opened: OpenedFrame? = await engine.run { social in
    ...
    private func dispatchInboundFrame(_ type: UInt8, _ payload: Data, viaNearby: Bool, senderDevice: String?) {
        switch type {
        case 0: Task { @MainActor in _ = await self.handleHello(payload, viaNearby: viaNearby, senderDevice: senderDevice) }
    ...
    private func handleHistoryRequest(_ payload: Data) {
        guard payload.count > 72, let engine else { return }
    ...
    private func handleDeviceRosterAnnounce(_ payload: Data) {
        guard let engine else { return }
        Task { @MainActor [weak self] in
            // The ingest drains pending epoch envelopes and re-verifies device credentials (ML-DSA
            // key expansion per device) — the 1.4–1.9s main-thread blocks the stall detector caught
            // on the self-sync path came from this same call.
            let status = await engine.run { $0.ingestRosterWireStatus(wire: payload) }
            guard let self, self.engine === engine else { return }
            if status >= 0 {
                self.scheduleDeviceIdsFill()   // the read model learns the new device ids…
                self.dialTargetsCache.removeAll()   // …and they must be dialable now
    ...
    func requestDeviceEnrollment() {
        ...
        if engine != nil { sendIroh(24, p, to: myNodeHex) }  // also try the iroh path
    ...
    func enableDeviceRoster() {
        ...
        let engine = self.engine
        Task { @MainActor in
            await DeviceRosterManager.shared.enable(engine: engine, accountSeed: seed, accountBundle: bundle, accountHex: AccountStore.currentNodeHex())
        }
    }
    ...
    func revokeDevice(_ nodeHex: String) {
        guard let seed = AccountStore.storedSeed(), let engine else { return }
        Task { @MainActor [weak self] in
            guard await DeviceRosterManager.shared.revoke(nodeHex, engine: engine, accountSeed: seed) else { return }
            guard let self, self.engine === engine else { return }
            await self.rotateSelfSyncAfterRevocation(revokedHex: nodeHex, accountSeed: seed, engine: engine)
        }
    }

    // MARK: - Switch-Flip 1.0.7 (docs/SWITCH-FLIP-1.0.7.md) — turn the new crypto ON, per-circle-gated.
    //
    // The per-launch switches (§3 keying, §4 seed-drop retirement, §2 creator pins, §5 DM live
    // lanes) and the §1 account-leaf migration run in the boot pass — see `postBootPass`.
    ...
    private func rotateSelfSyncAfterRevocation(revokedHex: String, accountSeed: Data, engine: Engine) async {
        let capable = await engine.run { $0.accountLeafRetired() }   // proxy for "all own devices seed-drop-capable"
        guard self.engine === engine else { return }
        ...
        Task {
            ...
            _ = await SelfSyncCoordinator.shared.sync(engine: engine, force: true)
        }
    ...
    private func handleDeviceEnrollmentRequest(_ payload: Data) {
        ...
        let engine = self.engine
        Task { @MainActor [weak self] in
            await DeviceRosterManager.shared.enable(engine: engine, accountSeed: seed, accountBundle: accountBundle, accountHex: accountHex)
            guard let cred = await DeviceRosterManager.shared.addLinkedDevice(bundle: bundle, nodeHex: hex, name: name, engine: engine, accountSeed: seed) else { return }
            guard let self, self.engine === engine else { return }
            var grant = Data()
            self.lpAppend(&grant, Data(hex.utf8))
            self.lpAppend(&grant, cred)
            self.sendToMyDevices(25, grant)
    ...
    func confirmSeedlessEnroll(_ request: PendingEnrollRequest) {
        ...
        let relays = RelayMailboxStore.shared.allRelays()
        Task { @MainActor [weak self] in
            await DeviceRosterManager.shared.enable(engine: engine, accountSeed: seed, accountBundle: accountBundle,
                                                    accountHex: accountHex)
            _ = await DeviceRosterManager.shared.addLinkedDevice(bundle: request.deviceBundle, nodeHex: request.deviceHex,
                                                                 name: request.name, engine: engine, accountSeed: seed)
            // The primary-signed roster WIRE (verbatim, incl. capability trailer) the new device installs.
            let rosterWire = await engine?.run { $0.myDeviceRosterWire() } ?? Data()
            guard let self, self.engine === engine else { return }
            ...
            self.nearbyBroadcast(29, grant)
            self.sendToDeviceHex(29, grant, request.deviceHex)   // directed to the requester
            self.activeEnrollTicket = nil                          // single-use — consume it
    ...
    private func sendToMyDevices(_ type: UInt8, _ payload: Data) {
        ...
        if engine != nil { sendIroh(type, payload, to: myNodeHex) }
    }
    ...
    private func pushFullStateToMyDevices() {
        ...
        Task { @MainActor [weak self] in
            let slot = await SelfSyncCoordinator.shared.sealedLocalSlot(engine: engine)
            // The full re-seal of every circle's history — a signature per event, on the engine actor.
            let history: [(cid: String, envs: [Data])] = await engine.run { s in
            ...
            for (cid, envs) in history {
                for env in envs {
                    let payload = self.eventPayload(cid, env)
                    if cid.hasPrefix("dm:") {
                        ...
                        self.sendIroh(1, payload, to: myHex)
                    } else {
                        self.sendToMyDevices(1, payload)
                    }
                }
            }
            self.refresh()
    ...
    func shareBucketWithCircle() {
        guard engine != nil, let s3 = SharedStore.ownerS3() else { return }
        let cid = activeCircleId
    ...
    private func handleBucketConfig(_ payload: Data) {
        ...
        Task { @MainActor [weak self] in
            let data = await engine.run { $0.openCircleMedia(circleId: circleId, sealed: sealed) }
            guard let self, self.engine === engine, let data,
                  let cfg = try? JSONDecoder().decode(S3Config.self, from: data) else { return }
            SharedMailboxStore.shared.set(cfg)
            self.pollMailboxNow()   // immediately pull from the newly-shared bucket
        }
    }
    ...
    func relayLink(forCircle cid: String? = nil) -> String? {
        ...
        var members = cachedMembers(circle)
    ...
    func adoptRelayNode(_ nodeHex: String, circleIds: [String], setDefault: Bool) {
        ...
        hex = hex.lowercased()
        guard let engine, hex.count == 64 else { return }
        ...
        Task { @MainActor [weak self] in
            // Seal the announce to every circle in one pass (real crypto per circle), then fan out.
            let sealedPerCircle: [(cid: String, sealed: Data)] = await engine.run { s in
                circleIds.compactMap { cid in (try? s.sealCircleMedia(circleId: cid, data: data)).map { (cid, $0) } }
            }
            guard let self, self.engine === engine else { return }
            for (cid, sealed) in sealedPerCircle {
                var p = Data(); self.lpAppend(&p, Data(cid.utf8)); p.append(sealed)
                let members = self.dialTargets(cid)
                for m in members { self.sendIroh(19, p, to: m) }
                self.nearbyBroadcast(19, p)
                self.originateRelay(dests: members, inner: self.frame(19, p))
            }
            ...
            self.backfillMailbox(circleIds: circleIds)
            Task { await BackgroundUploader.shared.flush() }
            self.pollMailboxNow()
        }
    }
    ...
    private func handleRelayNode(_ payload: Data) async {
        ...
        let read: (data: Data, members: [String])? = await engine.run { s in
            guard let opened = s.openCircleMediaSender(circleId: circleId, sealed: sealed) else { return nil }
            return (opened.data, s.contactNodeIds(circleId: circleId))
        }
        guard self.engine === engine, let read, var nodeHex = String(data: read.data, encoding: .utf8) else { return }
        let data = read.data
    ...
    func circleMemberships() -> [(String, [String])] {
        ...
        return circles.map { c in
            ...
            for a in accounts {
                ...
                for d in cachedDeviceIds(a) where !ids.contains(d) { ids.append(d) }
            }
            return (c.id, ids)
        }
    }
    func sealCirclePresign(circleId: String, data: Data) async -> Data? {
        guard let engine else { return nil }
        return await engine.run { try? $0.sealCircleMedia(circleId: circleId, data: data) }
    }
    func openCirclePresign(circleId: String, sealed: Data) async -> Data? {
        guard let engine else { return nil }
        return await engine.run { $0.openCircleMedia(circleId: circleId, sealed: sealed) }
    }
    ...
    func broadcastPresignBootstrap(circleId: String, getURL: String) {
        guard let engine, let data = getURL.data(using: .utf8) else { return }
        Task { @MainActor [weak self] in
            let sealed = await engine.run { try? $0.sealCircleMedia(circleId: circleId, data: data) }
            guard let self, self.engine === engine, let sealed else { return }
            var p = Data(); self.lpAppend(&p, Data(circleId.utf8)); p.append(sealed)
            let members = self.dialTargets(circleId)
            for m in members { self.sendIroh(20, p, to: m) }
            self.nearbyBroadcast(20, p)
            self.originateRelay(dests: members, inner: self.frame(20, p))
        }
    }
    ...
    private func handlePresignBootstrap(_ payload: Data) {
        ...
        Task { @MainActor [weak self] in
            ...
            self.backfillMailbox(circleIds: [circleId])             // re-upload my past posts to the pool
            Task { await BackgroundUploader.shared.flush() }   // deliver posts we couldn't send before
            self.pollMailboxNow()
        }
    }
    ...
    func verifyHeldMedia(limit: Int = 6) {
        ...
        // front of it was gone). Three hops now: the feed walk on the engine actor, the held-bytes
    ...
    func requestMediaWhenAvailable(ref: String, circleId: String, postId: String, authorShort: String,
        ...
        if authorShort.isEmpty || myNodeHex.lowercased().hasPrefix(authorShort.lowercased()) {
            guard let engine, MediaStore.shared.has(ref) else {
    ...
    private func handleMediaWanted(_ payload: Data) {
        guard payload.count > 64, let engine else { return }
        ...
        Task { @MainActor in
            ...
            let ok = await SharedStore.backup(ref: ref, circleId: circleId, engine: engine, force: true, reseal: true)
            guard ok else {
    ...
    func myOtherDeviceHexes() -> [String] {
        ...
        return cachedDeviceIds(account)
            .map { $0.lowercased() }
    ...
    func sendCallFrame(_ type: UInt8, _ payload: Data, to nodeHex: String) {
        ...
        guard let engine else { return }
        Task.detached(priority: .userInitiated) {
            let sealedOpt = await engine.run {
                try? $0.sealCallFrame(recipientNodeHex: nodeHex, frameType: type, data: payload)
            }
            guard let sealed = sealedOpt, !sealed.isEmpty else {
                let known = await engine.run { $0.deviceNodeIdsFor(accountHex: nodeHex).count }
    ...
    fileprivate func deliverSealedCallFrame(_ type: UInt8, _ sealed: Data, to nodeHex: String) {
        ...
        var dests = engine == nil ? [] : cachedDeviceIds(nodeHex)
    ...
    func pushCallInvite(to nodeHex: String, callerName: String) {
        guard let engine else {
            HavenLog.call("pushCallInvite SKIPPED for \(nodeHex.prefix(8)) — no engine yet")
            return
        }
        ...
        Task { @MainActor in
            let sealed = await engine.run { try? $0.sealSignedNotification(recipientNodeHex: nodeHex, data: json) }
    ...
    func requestMedia(_ ref: String, circleId: String? = nil) {
        guard let engine, !MediaStore.isSynthetic(ref), !MediaStore.shared.has(ref) else { return }
        #if os(iOS)
    ...
    func nudgeMediaPrefetchNow() {
        ...
        if let engine { MediaBackupQueue.shared.drainPersisted(engine: engine) }
    }
    ...
    private func requestMissingMedia() {
        ...
        guard appIsForeground else { return }
        #endif
        guard let engine, node != nil || nearby != nil else { return }
    ...
    private func sendMediaChunks(ref: String, fileURL url: URL, to requesterHex: String, missing: Set<Int>? = nil) {
        guard let engine, let handle = try? FileHandle(forReadingFrom: url) else { return }
        ...
        Task.detached(priority: .utility) { [weak self] in
            ...
            for index in 0..<total {
                ...
                guard let sealed = await engine.run({ try? $0.sealMedia(recipientNodeHex: requesterHex, data: chunk) }) else {
                ...
                let sent = mesh?.broadcastWaiting(out, class: .bulk, maxWaitMs: 8_000) ?? false
                if !sent {
    ...
    private func handleMediaChunk(_ payload: Data) {
        guard engine != nil, payload.count >= 2 else { return }
        ...
        Self.mediaQueue.async { [weak self] in
            ...
            if let plain {
                ...
                // KEM (friend) open needs the engine → on its actor, then the bookkeeping on main (rare path).
                Task { @MainActor in
                    guard let self, let engine = self.engine else { return }
                    guard let kp = await engine.run({ $0.openMedia(sealed: sealed) }) else { return }
                    if let fh = try? FileHandle(forWritingTo: tempURL) {
    ...
    ///
    /// Async since the read model: the verification, the device→account lookup and the circle
    /// membership check are one engine READ pass; the decisions run on the main actor against the
    /// contact / block / tombstone stores; the circle create + bundle add (and the first-page re-seal
    /// for the reply) are a WRITE pass. Every one of those used to run on main per hello, and a
    /// mailbox pass hands over hellos in bursts.
    @discardableResult
    private func handleHello(_ payload: Data, viaNearby: Bool = false, senderDevice: String? = nil) async -> (consumed: Bool, why: String) {
        guard let engine else { return (false, "engine-not-ready") }
        ...
        if idHex == myNodeHex {
            ...
            if SeedlessState.isEnabled, !profileBlob.isEmpty { await engine.run { $0.setCachedProfile(blob: profileBlob) } }
            if let slot = await SelfSyncCoordinator.shared.sealedLocalSlot(engine: engine) { nearbyBroadcast(23, slot) }
            return (true, "self-device")
        }
        // ONE read pass for everything the decisions below ask the engine.
        struct HelloReads {
            let account: String?        // the account this device id belongs to (device→account), if known
            let engineKnows: Bool       // the engine already lists this account in one of my circles
        ...
        if let account = reads.account, account != idHex.lowercased() {
        ...
        if !isContact(idHex) {
            ...
            if reads.engineKnows {
                let name = reads.name ?? "Someone"
                ContactsStore.shared.add(name: name.isEmpty ? "Someone" : name, idHex: idHex, verificationHex: reads.vhex)
        ...
        //
        // The re-seal is a signature per event — engine work, on its actor.
        let firstPage: [Data] = await engine.run { s in
    ...
    private func isOwnDeviceHex(_ hex: String) -> Bool {
        ...
        if nowMs &- ownDeviceHexCache.at > 15_000 || ownDeviceHexCache.set.isEmpty {
            guard engine != nil else { return false }
            var s = Set(cachedDeviceIds(myNodeHex).map { $0.lowercased() })   // the read model, never the engine
    ...
    private func runCircleSideEffects(_ cids: Set<String>) async {
        ...
        let nowMs = now()
        var gens: [String: UInt64] = [:]
        for cid in cids { gens[cid] = messagesGeneration[cid] ?? 0 }
        // One feed() per touched circle, serialized with other engine work — never on main.
        let feeds: [String: [FeedItemFfi]] = await engine.run { s in
            var out: [String: [FeedItemFfi]] = [:]
            for cid in cids {
                out[cid] = s.feed(circleId: cid, nowMs: nowMs,
        ...
        for cid in cids {
            let raw = feeds[cid] ?? []
            storeMessages(cid, raw, readAt: nowMs, generation: gens[cid])   // the read model, published
        ...
        // Refresh the in-app activity list off this same coalesced pass (its own engine hop,
